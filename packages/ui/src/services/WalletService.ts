...
export class WalletService {
  constructor(private walletManager: WalletManager, private component: HTMLElement) {}
  ...
  async connectWallet(walletId: string, options?: any) {
      ...
      try {
        ...
        if (walletId === 'walletconnect') {
          ...
          (this.component as any).showAccountSelectionView(walletId, wallet.name, wallet.icon, accounts);
        } else {
        ...
        (this.component as any).showErrorView(walletId, wallet?.name || 'Wallet', new Error(errorMessage));
        this.component.dispatchEvent(new CustomEvent('error', { detail: { error, walletId, errorType } }));
  ...
  async connectWithLedgerAccount(accountIndex: number) {
    if (!this.walletManager || !(this.component as any).accountSelectionData) return;
      ...
      try {
        ...
        this.component.dispatchEvent(new CustomEvent('connecting', { detail: { walletId, accountIndex } }));
        ...
        this.component.dispatchEvent(new CustomEvent('connected', { detail: { walletId, accountIndex } }));
      } catch (error: any) {
  ...
  async connectWithCustomDerivationPath(derivationPath: string) {
    if (!this.walletManager || !(this.component as any).accountSelectionData) return;
      ...
      try {
        ...
        this.component.dispatchEvent(new CustomEvent('connecting', { detail: { walletId, derivationPath } }));
        ...
        this.component.dispatchEvent(new CustomEvent('connected', { detail: { walletId, derivationPath } }));
      } catch (error: any) {
        // Handle error - show error view
        let errorMessage = error.message || 'An unexpected error occurred';
        let errorType: 'rejected' | 'unavailable' | 'failed' = 'failed';
