...
- Core / UI: wallet availability checks are now bounded by a timeout. `checkWalletAvailability()` (UI modal) and `WalletManager.getAvailableWallets()` could both wait indefinitely for an adapter's `isAvailable()` call, so one slow or hung check (e.g. a network probe on a flaky mobile connection) could stall the whole list and leave the connect modal blank/frozen. Checks now run in parallel and are raced against a 1000 ms timeout (`TIME.AVAILABILITY_TIMEOUT`) via a new `withTimeout` helper — a wallet that doesn't answer in time is treated as unavailable and the modal renders the rest of the list (#16).
