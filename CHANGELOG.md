...
- Adapters (`@xrpl-connect/adapter-xaman`): `sign()` and `signAndSubmit()` now pass `options.submit` explicitly (`false` / `true`) to Xaman's payload API, instead of the bare transaction with no `options` at all — previously whether Xaman submitted to the ledger was left entirely to the connected account's own auto-submit setting, regardless of which method was called. Both methods also now fetch `payload.get(uuid)` after the websocket confirms `signed: true` to populate `tx_blob`/`signature`/`tx_json` — the websocket push itself only ever carries `{ signed, txid }`, never the signed blob, so those fields were previously always empty (#105).
