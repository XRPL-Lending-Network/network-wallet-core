...
- Adapters (`@xrpl-connect/adapter-xaman`): `sign()` and `signAndSubmit()` now pass `options.submit` explicitly (`false` / `true`), use Xaman's managed payload subscription without leaking a second websocket, and validate the authoritative resolved payload before returning its transaction hash, blob, signature, and JSON. Sign-only requests fail if Xaman dispatches them, while submit requests return only after Xaman reports that the transaction was applied or accepted into the queue (#105).
