...
- Adapters (`@xrpl-connect/adapter-xaman`): `sign()` and `signAndSubmit()` now pass `options.submit` explicitly (`false` / `true`), force and validate the connected network, use Xaman's managed payload subscription without leaking a second websocket, and validate the authoritative resolved payload's transaction hash, blob, signature, and JSON. Opened requests can resolve after payload expiry, disconnecting cancels local subscriptions, sign-only requests fail if Xaman dispatches them, and submit requests return only after Xaman reports that the transaction was applied or accepted into the queue (#105).
