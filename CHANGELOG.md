...
- Core / UI: wallet availability checks are now bounded by a timeout. `checkWalletAvailability()` (UI modal) and `WalletManager.getAvailableWallets()` ran `isAvailable()` for every adapter inside a `Promise.all` with no time limit, so a single slow or hung check (e.g. a network probe on a flaky mobile connection) would stall the whole list and could leave the connect modal blank/frozen. Each check is now raced against a 1000 ms timeout (`TIME.AVAILABILITY_TIMEOUT`) via a new `withTimeout` helper — a wallet that doesn't answer in time is treated as unavailable and the modal renders the rest of the list (#16).
