...
- Regression coverage: assert the ESM and UMD output syntax, then install the real tarballs into an isolated consumer and build the umbrella ESM with Vite 7/Rollup 4.
- Verification: `pnpm exec vp check`, `pnpm test`, and `pnpm --filter xrpl-connect test:publish` pass. The publish test covers package builds, pack/dry-run checks, strict consumer install, type/runtime checks, both emitted artifact forms, and the Vite production build.
